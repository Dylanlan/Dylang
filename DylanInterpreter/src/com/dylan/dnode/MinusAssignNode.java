...
public class MinusAssignNode implements DNode {
	...
	public MinusAssignNode(String name, DNode value) {
		this.name = name;
		this.value = value;
	}
	...
	public DValue evaluate(Scope currentScope) {
		...
		if (vs != null) {
			BinaryOperationNode result = new BinaryOperationNode(new AtomNode(vs.getValue()), this.value, BinaryOperationNode.BON_SUB);
